...
function verificarLetraClicada() {
  ...
  botoesTecla.forEach(buttonClicado => { // Adiciona um listener (evento) de clique a cada botão
    buttonClicado.addEventListener("click", () => { // Ao clicar em qualquer botão:
      acionaBotaoDica(); // Chama a função que oculta o título e orientações
      bloquearTeclasExceto(buttonClicado); // Bloqueia todas as teclas, exceto a ativa ou já clicada

      const letra = buttonClicado.id; // Obtém a letra do id do botão clicado
      let acertou = false; // Variável para rastrear se a letra foi encontrada
      let countMatches = 0; // Conta quantas vezes a letra foi encontrada
      ...
      document.querySelectorAll("input.box").forEach(inputBox => { // Seleciona todos os inputs com a classe "box"
        if (inputBox.dataset.letra === letra) { // Compara o atributo data-letra do input com a letra clicada
          inputBox.value            = letra;
          ...
          countMatches++; // Incrementa o contador de acertos
        }
      });
